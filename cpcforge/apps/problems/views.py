...
from utils import db_entry_point, judge
...
class CreateProblemAPI(GenericAPIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            db_entry_point['platforms'].find_one_and_update(
...
class SubmitProblemAPI(GenericAPIView):
    ...
    def post(self, request):
        ...
        problem_lst = db_entry_point['platforms'].find_one({ 'handle': questn_data['pltHandle'] })['problems']
        for prob in problem_lst:
            # Finding the specific problem of the given handles
            if prob['handle'] == questn_data['probHandle']:
                result = judge.solve_tcases(questn_data['writtenCode'], prob['testCasesIn'], prob['testCasesOut'])
                db_entry_point['submit_logs'].insert_one({
